...
def readExperimentFile():
    ...
    with open(path,'r') as csvfile:
        ...
        individualOreState = 0
        test = 1
        for experiment in measure:
            
            if (experiment[0] == str(test) ):
          
                if ("ORESTATE" in experiment):
                    individualOreState = float(experiment[4])
                ...
                if ("ORESTATE" in experiment):
                    print("Inside")
                    if (experiment[1] not in oreStates.keys()):
                        oreStates[experiment[1]] = [individualOreState]
                    else:
                        oreStates[experiment[1]].append(individualOreState)
    print(oreStates)
